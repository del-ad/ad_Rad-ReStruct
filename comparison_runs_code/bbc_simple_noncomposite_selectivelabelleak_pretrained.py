...
### to be used with simple FF classifier instead of fusion one, pretrained FF classifier
...
def forward(model, img_and_global, input_ids, q_attn_mask, attn_mask, token_type_ids_q=None, batch_metadata=None, mode='train'):
        ...
        if model.args.use_precomputed:
            ...
            for batch_meta, batch_pred_dict in zip(batch_metadata, labels_to_preds):
                #total = 
                for pos_option_label in batch_meta['positive_option']:
                    #model.total += 1
                    setattr(model, f"{mode}_total", getattr(model, f"{mode}_total") + 1)
                    if batch_pred_dict[pos_option_label]:
                        #model.positive += 1
                        setattr(model, f"{mode}_positive", getattr(model, f"{mode}_positive") + 1)
                    else:
                        #model.negative += 1
                        setattr(model, f"{mode}_negative", getattr(model, f"{mode}_negative") + 1)
                    #print("hi")
            tot = getattr(model, f"{mode}_total")
            pos = getattr(model, f"{mode}_positive")
            neg = getattr(model, f"{mode}_negative")
            ...
            for idx, batch in enumerate(batch_metadata):
                ...
                for option,tensor in image_options_embedding_dict[idx].items():
                    if labels_to_preds[idx][option]:
                        g_embed = global_embedding[idx:idx+1]
                        image_options_embedding_dict[idx][option] = g_embed
                        del tensor
