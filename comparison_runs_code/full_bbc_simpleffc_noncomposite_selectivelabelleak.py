...
### simple FF classifier, selective label leak, nothing frozen

# python -m train.train_radrestruct \
#     --run_name "bbc_noncomposite_embedding_selectivelabelleak" \
...
#     --use_kb_adapter \
#     --initialize_kb \
#     --kb_adapter_dir /home/guests/adrian_delchev/code/ad_Rad-ReStruct/checkpoints_radrestruct/kb_bc_train/epoch=26-F1/val=0.87.ckpt \
...
def forward(model, img_and_global, input_ids, q_attn_mask, attn_mask, token_type_ids_q=None, batch_metadata=None, mode='train'):
        ...
        if model.args.use_precomputed:
            ...
            if model.args.use_kb_adapter:
                input_tokens, ttids, attn_masks, gt_labels, batch_options = generate_simpleffn_bbc_input(model, cls_tokens, image_features, \
                ...
                out = model.bbc_simple_ffn(input_tokens)
                # h_log = out['last_hidden_state']
                # logits = model.bbc_classifier(h_log.mean(dim=1))
                preds_bbc = out.sigmoid().detach()
                score_boosts, labels_to_preds = get_score_boosts(preds_bbc, gt_labels, batch_options)
            
            # ### bbc bias
            # knowledge_sequence, knowledge_ttid = generate_knowledge_sequence_composite_precomputed_bbcbias(model, options_embeddings_image=image_options_embedding_dict,
            ...
            # selectively leak labels (like in sanity check but based on bbc)
            for idx, batch in enumerate(batch_metadata):
                #positive_options = set(batch['positive_option'])
                for option,tensor in image_options_embedding_dict[idx].items():
                    if labels_to_preds[idx][option]:
                        image_options_embedding_dict[idx][option] = global_embedding[idx:idx+1]
                        del tensor
            ...
            #*** equalize that to precomputed version
            kb_examples = model.knowledge_base.get_images_for_paths(batch_metadata)
            image_options_embedding_dict = produce_option_embeddings_batched(kb_examples=kb_examples, image_encoder=model.image_encoder)
            ...
            input_tokens, ttids, attn_masks, gt_labels, batch_options = generate_simpleffn_bbc_input(model,\
                                        global_embedding, image_options_embedding_dict, \
                                        batch_metadata, \
                                        model.knowledge_base.rrs_as_kb_train if mode=='train' else model.knowledge_base.rrs_as_kb_val)
            
            
            ### simple bbc predictions            
            out = model.bbc_simple_ffn(input_tokens)
            preds_bbc = out.sigmoid().detach()
            score_boosts, labels_to_preds = get_score_boosts(preds_bbc, gt_labels, batch_options, batch_metadata)
            
            
            ### selective label leak
            for idx, batch in enumerate(batch_metadata):
                #positive_options = set(batch['positive_option'])
                for option,tensor in image_options_embedding_dict[idx].items():
                    if labels_to_preds[idx][option]:
            ...
            score_boosts = []
            knowledge_sequence, knowledge_ttid = generate_knowledge_sequence_fix(model, options_embeddings_image=image_options_embedding_dict,
                                                                    options_embedding_text=options_embedding_text,
                                                                    col_embedding=col_embedding,
                                                                    separator_embedding=seperator_embedding,
                                                                    batch_metadata=batch_metadata,
...
def training_step(model, batch, batch_idx, dataset="vqarad"):
    ...
    if model.args.use_precomputed:
        ...
        out, _, bbc_preds, bbc_gt = model(img, question_token, q_attention_mask, attn_mask, token_type_ids_q, info, mode='train')
...
def validation_step(model, batch, batch_idx):
    ...
    if model.args.use_precomputed:
        ...
        out, _, bbc_pred, bbc_gt = model(img, question_token, q_attention_mask, attn_mask, token_type_ids_q, batch_info, mode='val')
...
### HELPERS
def generate_knowledge_sequence_fix(model, options_embeddings_image, options_embedding_text, col_embedding, separator_embedding, batch_metadata, use_noise='no'):
...
def generate_full_batch_sequence_bbc_noncomposite(args, cls_tokens, image_features, \
                                    global_embeddings, image_options_g_embeddings, \
                                    text_features, seperator_embedding, \
                                    pad_embedding, token_type_ids_q, \
                                    batch_metadata, attn_mask, \
...
def generate_simpleffn_bbc_input(model, global_embeddings, \
                                image_options_g_embeddings, batch_metadata, \
                                rrs_as_kb):
    """
    This version pre-allocates a single tensor of shape
    (batch_size, max_position_embeddings, 768)
    ...
    hidden_dim_size = model.args.hidden_size  # typically 768
    total_new_samples = 0
    ...
    generated_batch = torch.zeros((num_options_in_batch, hidden_dim_size * 2), device=global_embeddings.device)
        ...
        for option in batch_metadata[batch_idx]['options']:
            ...
            if option == 'yes' and kb_example_embeddings[option].size(0) > 1:
                ...
                for l1_option in range(kb_example_embeddings[option].size(0)):
                    ...
                    #write_pos = 0
                    ## Knowledge image embedding
                    knowledge_global_embedding = kb_example_embeddings[option][l1_option: l1_option + 1].to(device=global_image_embedding.device)
                    generated_batch_sample = torch.cat([global_image_embedding, knowledge_global_embedding], dim=-1)
                    
                    generated_batch[write_pos] = generated_batch_sample
                    write_pos += 1
            ...
            # L2 + L2 questions
                ...
                knowledge_global_embedding = kb_example_embeddings[option].to(device=global_image_embedding.device)
                generated_batch_sample = torch.cat([global_image_embedding, knowledge_global_embedding], dim=-1)
    ...
    gt_for_batch = torch.cat(
        [torch.tensor(lst, dtype=torch.float32) if not isinstance(lst, torch.Tensor) else lst.float() for lst in
         gt_for_batch]).unsqueeze(1)
    return generated_batch,[],[], gt_for_batch, batch_options

def generate_knowledge_sequence_composite_precomputed_bbcbias(model, options_embeddings_image, g_embedding_image, \
...
def get_global_embeddings_in_batches(encoder, images, max_batch_size, device):
    """
...
def produce_option_embeddings_batched(kb_examples: List[Dict], image_encoder):
    ...
    try:
        for batch_idx, single_batch in enumerate(kb_examples):
            for option, images in single_batch.items():
                if images:  # Skip options with no images
                    ...
                    is_batched_list = []
                    for img in images:
                        if img.dim() == 3 and img.shape[0] == 3: ### for L2 and L3 images - each option consists of n single images
                            flattened_images.append(img)
                            sizes.append(1)
                            is_batched_list.append(False)
                        elif img.dim() == 4 and img.shape[1] == 3: ### for L1 images - only a single option (composite) consisting of n images
                            n = img.shape[0]
                            for i in range(n):
        ...
        if len(image_batches) > 0:
            ...
            with torch.no_grad():
                global_embeddings = get_global_embeddings_in_batches(image_encoder, batched_images_gpu, \
...
def get_score_boosts(predictions, gt_labels, batch_options, batch_metadata):
    ...
    for batch_idx, batch in enumerate(batch_options):
        ...
        ### single choice question where the answer options are mutually exclusive
        if batch_metadata[batch_idx]['answer_type'] == 'single_choice':
            # Mutually exclusive: only boost the highest-scoring label (if any)
            best_label = None
            best_score = -float('inf')
            for label, preds in label_to_preds.items():
                max_pred = max(preds)
                ## always boost highest pred regardless if it passes the threshold
                # if max_pred >= SIMILARITY_THRESHOLD and max_pred > best_score:
                #     best_score = max_pred
                #     best_label = label
                if max_pred > best_score:
                    best_score = max_pred
                    best_label = label
            if best_label:
            ...
            for label, preds in label_to_preds.items():
                if any(pred >= SIMILARITY_THRESHOLD for pred in preds): ## only boost if it passes the 0.5 threshold
...
def generate_knowledge_sequence_vanilla_precomputed(model, options_embeddings_image, options_embedding_text, col_embedding, separator_embedding, batch_metadata, use_noise='no'):
    ...
    for batch_index in range(len(options_embedding_text)):
        ...
        for option in batch_metadata[batch_index]['options']:
            ...
            ### in vanilla mode we have yes:1x1768 img embedding - a composite embedding for L1 questions
            if img_emb.size(0) > 1:
                img_emb, _ = torch.max(img_emb, dim=0, keepdim=True)
