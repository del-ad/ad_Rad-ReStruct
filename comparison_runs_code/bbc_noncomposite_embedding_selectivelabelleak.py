...
### for L1 questions - the example consists of a non composite image, with 1 positibe example for each l2 option (l2 positive option is composite image of 3)
### This means that the BBC creates extra samples for each batch
### Fusion Model ###
...
### IMG_OPTION_GLOBAL_EMBEDDING is replaced with the INPUT_IMAGE_GLOBAL_EMBEDDING - the sanity check part, where predicted by the BBC
### Next steps: ### - pretrain BBC only and use a pretrained BBC, then frozen with a RRS fusion
###### Both models are trained together (BBC + RRS fusion)
...
def forward(model, img_and_global, input_ids, q_attn_mask, attn_mask, token_type_ids_q=None, batch_metadata=None, mode='train'):
        ...
        if model.args.use_precomputed:
            ...
            if model.args.use_kb_adapter:
                input_tokens, ttids, attn_masks, gt_labels, batch_options = generate_full_batch_sequence_bbc_noncomposite(model.args, cls_tokens, image_features, \
