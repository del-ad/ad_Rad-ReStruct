...
def forward(model, img_and_global, input_ids, q_attn_mask, attn_mask, token_type_ids_q=None, batch_metadata=None, mode='train'):
    ...
    input_tokens, ttids, attn_masks, gt_labels, batch_options = generate_simpleffn_bbc_input(model, [], image_features, \
    ...
    ffc_model_out = model.bbc_simple_ffn(input_tokens)
    ...
    if not torch.isfinite(ffc_model_out).all():
        print(f'{timestamp()}Fusion model produced nan/inf in ints output')

    return ffc_model_out, gt_labels, []
...
def training_epoch_end(model, outputs):
    """
    Aggregates predictions and targets for the epoch, computes and logs metrics, and clears accumulators.
    ...
    # 1. Combine all batch predictions and targets for this epoch
    preds = torch.cat(model.train_preds, dim=0).cpu().numpy()
    targets = torch.cat(model.train_targets, dim=0).cpu().numpy()
    
    # 2. Calculate standard binary metrics
    acc = accuracy_score(targets, preds)
    f1 = f1_score(targets, preds)
    
    # ROC AUC needs "probabilities" not hard predictions.
    # If you stored soft scores (model.train_soft_scores), use those.
    if hasattr(model, 'train_soft_scores') and len(model.train_soft_scores) > 0:
        probs = torch.cat(model.train_soft_scores, dim=0).cpu().numpy()
        try:
            auc = roc_auc_score(targets, probs)
        except ValueError:
            auc = float('nan')
    else:
        auc = float('nan')  # If you didn't collect soft scores

    # 3. Log metrics (replace with your logger as needed)
    model.log('train/accuracy', acc, on_epoch=True, prog_bar=True)
    model.log('train/f1', f1, on_epoch=True, prog_bar=True)
    model.log('train/auc', auc, on_epoch=True, prog_bar=True)

    #print(f"Epoch results -- Accuracy: {acc:.3f}, F1: {f1:.3f}, ROC AUC: {auc:.3f}")

    # 4. Clear accumulators for next epoch
    model.train_preds.clear()
    model.train_targets.clear()
    if hasattr(model, 'train_soft_scores'):
        model.train_soft_scores.clear()
    if hasattr(model, 'train_infos'):
        model.train_infos.clear()
...
def validation_epoch_end(model, outputs):
    ...
    Aggregates all predictions and targets, computes binary metrics, logs, and clears accumulators.
    """
    ...
    preds = torch.cat(model.val_preds, dim=0).cpu().numpy()
    targets = torch.cat(model.val_targets, dim=0).cpu().numpy()

    # 2. Metrics: Accuracy and F1
    acc = accuracy_score(targets, preds)
    f1 = f1_score(targets, preds)
    
    # 3. ROC AUC (using probabilities, not hard predictions)
    if hasattr(model, 'val_soft_scores') and len(model.val_soft_scores) > 0:
        probs = torch.cat(model.val_soft_scores, dim=0).cpu().numpy()
        try:
            auc = roc_auc_score(targets, probs)
        except ValueError:
            auc = float('nan')
    ...
    # 4. Log the metrics
    model.log('val/accuracy', acc, on_epoch=True, prog_bar=True)
    model.log('val/f1', f1, on_epoch=True, prog_bar=True)
    model.log('val/auc', auc, on_epoch=True, prog_bar=True)

    #print(f"Validation Epoch -- Accuracy: {acc:.3f}, F1: {f1:.3f}, ROC AUC: {auc:.3f}")

    # 5. Clear the accumulators
    model.val_preds.clear()
    model.val_targets.clear()
    if hasattr(model, 'val_soft_scores'):
        model.val_soft_scores.clear()
    if hasattr(model, 'val_infos'):
        model.val_infos.clear()

    model.val_preds = []
    model.val_targets = []
    model.val_answer_types = []
    model.val_gen_labels = []
    model.val_gen_preds = []
    model.val_infos = []
...
def generate_simpleffn_bbc_input(model, cls_tokens, image_features, \
                                    global_embeddings, image_options_g_embeddings, \
                                    text_features, seperator_embedding, \
                                    pad_embedding, token_type_ids_q, \
                                    batch_metadata, attn_mask, \
