...
def validation_epoch_end(model, outputs):
    ...
    preds = torch.cat(model.val_preds, dim=0)
    targets = torch.cat(model.val_targets, dim=0)

    if "vqarad" in model.args.data_dir:
        correct = (preds == targets).sum().item()
        total = targets.numel()
        accuracy = correct / total
        model.log('Accuracy/val', accuracy, on_epoch=True, prog_bar=True, logger=True)
    else:
        correct = (preds == targets).float().sum()
        total = torch.numel(targets)
        accuracy = correct / total
    ...
    try:
        ...
        model.log('F1/val', f1, on_epoch=True, prog_bar=True, logger=True)
    except ImportError:
    ...
    if hasattr(model, "val_soft_scores"):
        model.val_soft_scores = []
    if hasattr(model, "val_answer_types"):
